...
model_enemy4 = pygame.image.load("img/wall.png").convert()
...
class Boss(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.type == 2:
            if self.rect.x < 1920-self.rect.width and self.canShoot:
                wall = Enemy(4,self.rect.x,self.y)
                enemy_group.add(wall)
                coin = Coin(1,self.rect.x,self.y)
                coin_group.add(coin)
                self.canShoot = False
            if self.test == False:
                if pygame.time.get_ticks() - self.timer >6000:
...
class Enemy(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.rect.x+5 < 0-self.rect.width:
            self.kill()
        if self.type == 4:
              if player2.player_rect.colliderect(self.rect):
                    self.kill()
        ...
        while player1.Alive:
            ...
            if keys[pygame.K_RIGHT] and player1.player_rect.x < 200:
                ...
                for i in range(len(allEnemyLists[level_selected])):
                    if allEnemyLists[level_selected][i].type == 4:
            ...
            if level_selected == 1:
                ...
                if keys[pygame.K_SPACE] and player1.cooldown == False and player1.player_rect.x < 200 and nb_dash > 0:
                    nb_dash -= 1
                win.blit(boss2.image,boss2.rect)
